...
pass_managers = [
    generate_preset_pass_manager(optimization_level=level, backend=backend)
...
transpiler_services = [
    {
        "service": TranspilerService(
            optimization_level=3, ai=False, backend_name="ibm_brisbane"
        ),
        "ai": False,
    ...
    {
        "service": TranspilerService(
            optimization_level=3, ai=True, backend_name="ibm_brisbane"
        ),
        "ai": True,
        "optimization_level": 3,
    },
...
for circuit in circuits:
    sample_task_references.append(
        [transpile_parallel(circuit, config) for config in configs]
    )
...
save_result(
    {"transpiled_circuits": results, "execution_time": execution_time_serverless}
)
